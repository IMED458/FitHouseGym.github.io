...
<head>
  ...
  <script type="module">
    ...
    // მთავარი გასწორებული ფუნქცია — ვიზიტების შემცირება ყველა ტიპზე, რომელსაც აქვს remainingVisits
    window.processCheckIn = async function(id) {
      ...
      if (m.remainingVisits !== null && m.remainingVisits !== undefined) {
        ...
        if (updated.remainingVisits <= 0) {
          ...
          showToast("შესვლა დაფიქსირდა! ვიზიტები ამოიწურა — აბონემენტი დასრულდა", 'warning');
    ...
    window.checkMemberAccess = async function(member) {
      ...
      if (member.status !== 'active') {
        allowed = false; msg = 'შეჩერებულია';
      }
      else if (now > end) {
        allowed = false; msg = 'ვადა გასულია';
        ...
        allowed = false; msg = 'ვიზიტები ამოწურულია';
        await updateMember({...member, status:'expired'});
      }
      else if (member.subscriptionType === 'morning' && (hour < 9 || hour >= 16)) {
        allowed = false; msg = 'მხოლოდ 09:00–16:00';
      }
      ...
      document.getElementById('checkinResult').innerHTML = `
        <div class="member-card p-4">
          <div class="grid grid-cols-1 md:grid-cols-2 gap-2 text-sm">
            ...
            <div><strong>ჯამში ვიზიტები:</strong> ${member.totalVisits || 0}</div>
          </div>
  ...
  <script>
    const dejavuFont = "AAEAAAALAIAAAwAgT3JpZ...";
      ...
      <form id="registrationForm">
        ...
        <div id="customSubscriptionFields" style="display:none;margin-top:20px;padding:20px;background:#f7fafc;border-radius:12px;">
          <div class="form-grid">
            ...
            <input type="text" id="customDescription" class="form-input" placeholder="აღწერა (მაგ: 5 ვიზიტი)">
          </div>
        </div>
